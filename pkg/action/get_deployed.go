...
func processResourceForGetDeployed(ctx context.Context, manifest *yaml.RNode, dynamicClient *dynamic.DynamicClient,
	...
	gvr := dynamicClient.Resource(restMappingForGVK.Resource)
	// Get the resource's details from the cluster
	resource, err := getResource(ctx, gvr, name, namespace)
	if err != nil {
		return nil, err
	}
...
func getResource(ctx context.Context, gvr dynamic.NamespaceableResourceInterface,
	...
	if namespace != "" {
		resource, err := gvr.Namespace(namespace).
			Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return nil, fmt.Errorf("failed to get the namespaced resource: %w", err)
		}
	...
	resource, err := gvr.Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		return nil, fmt.Errorf("failed to get the non-namespaced resource: %w", err)
	}
